    ...
    $.ajax({
        ...
        success: function(data){
            ...
            if (no_update == undefined) {
                //msg_display("out", '<span class="dimgray">' + outmsg + '</span><br><br>');
                msg_display("out", outmsg + '<br><br>');
                history_add(outmsg);
                HISTORY_POS = 0;
...
function msg_display(type, msg){
    ...
    /*msg = $(msg).filter(function() { return this.nodeType == 3; }).each(function() {
        $(this).replace(/ /g, '&nbsp;')
        //var $this = $(this);
    ...
    $('#messagewindow').animate({scrollTop: $('#messagewindow')[0].scrollHeight}, 0); // Default speed = 400
}
